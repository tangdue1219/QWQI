    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "paragraph_idx": {"type": "integer", "description": "段落索引（从0开始）"},
                "paragraph_text":{"type": "string", "description": "被批注的段落原文（取前100字）"},
...
def _check_coread_pending(args: dict) -> str:
    ...
    if not sb:
        return "Supabase 未配置"
    res = sb.table("book_progress").select("book_id,current_chapter,updated_at").eq("reader", "qi_pending").execute()
    rows = res.data or []
    if not rows:
        return "暂无待共读章节。"
    results = []
